...
interface RoleCard  { icon: React.ComponentType<{ className?: string }>; role: string; desc: string; color: string; bg: string; gradient: string }
...
const ROLES: RoleCard[] = [
  { icon: Heart,   role: 'Patients', desc: 'Book appointments, view records, share documents and stay connected with your care team.',  color: 'text-emerald-600', bg: 'bg-emerald-50 dark:bg-emerald-950/30', gradient: 'from-emerald-500 to-teal-500'   },
  { icon: UserPlus,role: 'Doctors',  desc: 'Manage your schedule, access shared patient history, write notes and grow your practice.', color: 'text-violet-600',  bg: 'bg-violet-50 dark:bg-violet-950/30',   gradient: 'from-violet-500 to-purple-600'  },
  { icon: Shield,  role: 'Admins',   desc: 'Oversee users, verify doctor credentials, review documents and generate platform reports.', color: 'text-orange-600',  bg: 'bg-orange-50 dark:bg-orange-950/30',   gradient: 'from-orange-500 to-red-500'     },
];
...
const childVar = { hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 0.45 } } };
...
const LandingPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-neutral-950 overflow-x-hidden">
      ...
      {/* ── Navbar ─────────────────────────────────────────────────────────── */}
      <nav className="fixed top-0 inset-x-0 z-50 bg-white/80 dark:bg-neutral-950/80 backdrop-blur-lg border-b border-neutral-200/50 dark:border-neutral-800/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-bold text-neutral-900 dark:text-white text-lg">MedVault</span>
          </div>

          <div className="flex items-center gap-3">
            {isAuthenticated() && user?.role ? (
              <Link to={`/${user.role.toLowerCase()}/dashboard`}
                className="px-4 py-2 bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 rounded-xl text-sm font-semibold hover:opacity-90 transition-opacity">
                Go to Dashboard
              </Link>
            ) : (
              <>
                <Link to="/login" className="px-4 py-2 text-sm font-semibold text-neutral-700 dark:text-neutral-300 hover:text-neutral-900 dark:hover:text-white transition-colors">
                  Sign in
                </Link>
                <Link to="/register"
                  className="px-4 py-2 bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 rounded-xl text-sm font-semibold hover:opacity-90 transition-opacity">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-4xl mx-auto">
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              <span className="inline-flex items-center gap-1.5 py-1 px-3 rounded-full bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-400 text-sm font-semibold mb-6 border border-emerald-200 dark:border-emerald-800">
                <Star className="w-3.5 h-3.5 fill-current" /> Trusted by 10,000+ patients
              </span>
              ...
              <p className="text-xl text-neutral-500 dark:text-neutral-400 mb-10 max-w-2xl mx-auto leading-relaxed">
                Book appointments, manage records, and connect with verified doctors — all in one secure platform built for modern healthcare.
              </p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div {...fadeUp()} className="text-center mb-16">
            ...
            <p className="text-lg text-neutral-500 dark:text-neutral-400 max-w-2xl mx-auto">
              MedVault brings together the tools patients, doctors, and admins need in one cohesive platform.
            </p>
          </motion.div>
          ...
          <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-80px' }}
            ...
            {FEATURES.map(f => {
              const Icon = f.icon;
              return (
                <motion.div key={f.title} variants={childVar}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div {...fadeUp()} className="text-center mb-16">
            ...
            <p className="text-lg text-neutral-500 dark:text-neutral-400 max-w-xl mx-auto">
              MedVault adapts to who you are — patient, doctor, or admin.
            </p>
          </motion.div>
          ...
          <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true }} className="grid md:grid-cols-3 gap-5">
            {ROLES.map(r => {
              const Icon = r.icon;
              return (
                <motion.div key={r.role} variants={childVar}
          ...
          <div className="space-y-5">
            {HOW_IT_WORKS.map((step, i) => (
              <motion.div key={step.step} {...fadeUp(i * 0.1)}
                ...
                <span className="text-4xl font-black text-neutral-100 dark:text-neutral-800 flex-shrink-0 leading-none">{step.step}</span>
                <div>
                ...
                <CheckCircle className="w-5 h-5 text-emerald-500 flex-shrink-0 mt-0.5 ml-auto" />
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div {...fadeUp()}
            ...
            <div className="absolute inset-0 overflow-hidden">
              <div className="absolute -top-1/2 -right-1/2 w-full h-full bg-gradient-to-b from-violet-500/20 to-emerald-500/20 blur-3xl" />
            </div>
            <div className="relative z-10">
              ...
              <p className="text-neutral-400 text-lg mb-8 max-w-xl mx-auto">
                Join thousands of patients and doctors already using MedVault every day.
              </p>
              {!isAuthenticated() && (
                <Link to="/register"
      ...
      <footer className="border-t border-neutral-200 dark:border-neutral-800 py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 bg-gradient-to-br from-emerald-500 to-violet-600 rounded-md flex items-center justify-center">
              <Activity className="w-3 h-3 text-white" strokeWidth={2.5} />
            </div>
            <span className="text-sm font-semibold text-neutral-700 dark:text-neutral-300">MedVault</span>
          </div>
          <div className="flex items-center gap-5 text-xs text-neutral-500">
            <Link to="/terms"   className="hover:text-neutral-800 dark:hover:text-neutral-300 transition-colors">Terms</Link>
            <Link to="/privacy" className="hover:text-neutral-800 dark:hover:text-neutral-300 transition-colors">Privacy</Link>
          </div>
          <p className="text-xs text-neutral-400">© {new Date().getFullYear()} MedVault. All rights reserved.</p>
        </div>
      </footer>
    </div>
  );
